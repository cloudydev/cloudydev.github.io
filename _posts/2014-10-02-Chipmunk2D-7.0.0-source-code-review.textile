...
p(. 存储容器：数组（cpArray.c）和哈希表（cpHashSet.h）
...
p(. **DYNAMIC**: 有一定质量的物体，外力施加到上面或发生碰撞时会改变运动状态
...
p(. **stamp** ：当前世界的时间
**dynamicBodies** ：世界中所有动态物体列表 
**staticBodies** ：所有静态物体列表
**dynamicShapes** ： 动态物体的加速结构
**staticShapes** ：静态物体的加速结构
...
p(. 1. X 轴向上的 Sweep and Prone，实现在 cpSweep1D.c 里
2. 空间网格划分，实现在 cpSpatialHash.c 里
3. 体积树，Chipmunk2D 里默认使用的结构，实现在 cpBBTree.c 里
...
p(. **idleSpeedThreshold**: 速度阀值，当物体的速度小于这个值时，可以近似的认为这个物体是静止的。
