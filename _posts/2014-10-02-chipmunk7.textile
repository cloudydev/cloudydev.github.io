...
space 是存储所有物体的容器，作为物体之间发生碰撞的环境（代码只贴了一部分，其他成员在下文会有介绍）：

**stamp**: 当前世界的时间
**dynamicBodies, staticBodies**: 动态、静态物体列表
**dynamicShapes, staticShapes**: 动态，静态物体的加速结构
...
* chipmunk 7 里提供了三种加速结构体：

p(. 1. cpSweep1D: 仅实现了在x轴向上的sweep and prone算法
2. cpSpatialHash: 空间网格划分
3. cpBBTree: 体积树，chipmunk默认使用

p.  所有的加速结构都需要实现 **cpSpatialIndexClass** 里声明的函数以提供基本的遍历、添加、删除、更新、碰撞查询操作，
...
/* 更新物体Obj的AABB，更新加速结构*/
typedef void (*cpSpatialIndexReindexObjectImpl)(cpSpatialIndex *index, 
                                      void *obj, 
                                      cpHashValue hashid);

/* 碰撞对查询，以查询到的碰撞对作为参调用回调函数func */
typedef void (*cpSpatialIndexReindexQueryImpl)(cpSpatialIndex *index, 
                                      cpSpatialIndexQueryFunc func,
                                      void *data);

/* 碰撞查询的回调*/
typedef cpCollisionID (*cpSpatialIndexQueryFunc)(void *obj1, 
                                                void *obj2, 
                                                cpCollisionID id, 
                                                void *data);
{% endhighlight %}
